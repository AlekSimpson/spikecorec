...
namespace {
    ...
    // linked into this extension) behind a single `SpikeEngine` member (its own NML-model
    // constructor — engine.h — now owns the allocate/build-weight-matrix/assemble/compile step this
    // class used to hand-roll itself, matching every other already-migrated call site's fold of
    // `nml::AssembledModel` into `SpikeEngine`). A Python caller only ever sees this class's
    // constructor plus a handful of run/read-back methods — everything else stays C++-internal. This
    // is deliberately the MINIMUM surface that satisfies "construct, run, and read back a GLIF
    // network from Python" — not a binding of the whole codegen pipeline.
    //
    ...
    public:
        NmlNetworkRunner(const string &nml_file_path, f32 dt_seconds)
            : model_(load_model(nml_file_path)),
              ...
              engine_(model_, programs_, dt_seconds),
        ...
        // No user-declared destructor: `engine_`/`stimulus_schedule_` each already free their own
        // GPU buffers via their own destructors (SpikeEngine::~SpikeEngine / StimulusSchedule::
        // ~StimulusSchedule), run in the usual reverse-declaration-order — unlike before this class
        // folded onto a single SpikeEngine member, there is no longer any buffer THIS class itself
        // allocated to free by hand.
        ~NmlNetworkRunner() = default;
        ...
        // dispatches one SpikeEngine::step_tick call.
        void step() {
            f32 *network_inputs = engine_.network_inputs.get_contents();
            for (s32 neuron_index = 0; neuron_index < model_.total_neuron_count; ++neuron_index) {
                network_inputs[neuron_index] += (f32)stimulus_schedule_.current_at(neuron_index, tick_);
            }
            engine_.step_tick(dt_seconds_, tick_, tick_ + 1);
            ++tick_;
        }
            ...
            for (s32 population_index = 0; population_index < (s32)model_.populations.size(); ++population_index) {
                ...
                for (s32 local_index = 0; local_index < population.size; ++local_index) {
                    destination[population.neuron_index_begin + local_index] =
                        engine_.nml_allocation_.cell_state.get_contents()[state_element_index(population_index, 0, local_index)];
                }
            }
            return result;
        }
        ...
        py::array_t<s64> last_spiked() const {
            prefetch_to_cpu(engine_.last_spiked, (usize)model_.total_neuron_count * sizeof(s64));
            return to_numpy(engine_.last_spiked.get_contents(), model_.total_neuron_count);
        }
    ...
    private:
        ...
        // One IrProgram per model.type_library entry, matching SpikeEngine's own NML-model
        // constructor convention. Mirrors examples/nml_pipeline_support.h's own
        ...
        s64 state_element_index(s32 population_index, s32 state_slot_index, s32 local_neuron_index) const {
            const nml::PopulationEntry &population = model_.populations[(usize)population_index];
            return engine_.nml_allocation_.cell_type_boundaries.get_contents()[population_index]
                   + (s64)state_slot_index * population.size + local_neuron_index;
        }

        // SpikeEngine's own NML-model constructor (which builds nml_allocation_) zero-initializes
        // cell_state and never evaluates a cell type's own OnStart (allocator.h/examples/
        // nml_pipeline_support.h) — every population whose cell type bakes an `EL` constant (every
        // GLIF variant's OnStart is `v = EL`) gets that seeded into its own `v` (state slot 0) by
        // hand here; a population whose cell type has no `EL` (out of this class's GLIF-shaped
        // scope) is left at that zero default rather than throwing.
...
PYBIND11_MODULE(_spikecorec, m) {
    ...
    // to end, via a single internal SpikeEngine (its own NML-model constructor). See
    // NmlNetworkRunner's own class comment above for the scope decision (this is the minimum viable
    // binding surface, not a full pipeline exposure).
