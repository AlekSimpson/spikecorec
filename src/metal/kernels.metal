...
#include "k2tree_device.metalinc"
...
kernel void neighbor_weights_kernel(
    ...
    constant long       &total_edge_count       [[ buffer(12) ]],
    constant long       &rank_float4_stride     [[ buffer(13) ]],
    const device float  *coefficients           [[ buffer(14) ]],
    const device long   *edge_row_offset        [[ buffer(15) ]],
    device float        *output_weights         [[ buffer(16) ]],
    uint thread_id [[ thread_position_in_grid ]]
) {
    // One thread per REAL edge, indexed by its canonical ordinal -- not one per padded
    // (node, slot) pair. There is no padding to skip and no sentinel to write, so every
    // thread does useful work and the output is exactly total_edge_count values.
    long edge_ordinal = (long)thread_id;
    if (edge_ordinal >= total_edge_count) return;

    // The source is the row whose offset range contains this ordinal. edge_row_offset is
    // sorted and has node_count + 1 entries, so a binary search for the last entry <=
    // edge_ordinal finds it in log(node_count) steps.
    long low = 0;
    long high = node_count;
    while (low < high) {
        long middle = (low + high + 1) / 2;
        if (edge_row_offset[middle] <= edge_ordinal) low = middle;
        else high = middle - 1;
    }
    long source_node = low;
    long neighbor_slot = edge_ordinal - edge_row_offset[source_node];
    ...
    if (target_node < 0) {
        output_weights[edge_ordinal] = 0.0f;
        return;
    }
    ...
    // Ck folded inline into the same accumulation loop and position the host uses, so the
    // two paths agree term for term rather than merely approximately.
    ...
    output_weights[edge_ordinal] = dot_product;
}
