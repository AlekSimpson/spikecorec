...
void AssembledModel::step_tick(const ModelRuntimeBuffers &buffers, f32 dt, s64 tick, s64 next_tick) {
    ...
    // here regardless of whether the population's cell type is closed-form-advanceable (arch §0.5,
    // ticket #62 [F1]'s own still-deliberate scope boundary -- see master_kernel.h's step_tick doc
    // comment): this is safe for a nonlinear population (never fast-forwarded/skipped, exactly the
    // correctness rule requires) and does not yet exploit closed-form-advanceable cell types for
    // their own optimization (a separate, future skip-dispatch ticket's job).
