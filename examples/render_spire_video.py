...
Large recordings are read lazily, and never through `spikecorec.read_spire_recording()` -- that
binding materializes an entire recording in RAM, which a big run makes impossible (a 1000x1000
torus recorded for 15000 ticks is 56 GiB per stream, so the spike + membrane pair together would
need 112 GiB). Neither path below needs the `spikecorec` extension module to have been built:
...
class StreamingSpireReader:
    ...
    56 GiB-decompressed recording renderable without either the `spikecorec` binding (whose
    read_spire_recording materializes every frame in RAM at once) or a built extension module.
...
def default_ticks_per_second_path(spike_recording_path):
    # Every recording example writes this file as "<same dir>/<base_name>_ticks_per_second.txt"
    # next to "<base_name>_spikes.spire[.gz|.xz|.bz2]" -- see nml_pipeline_support.h's
    # TicksPerSecondTelemetry. The telemetry file itself is always plain text, uncompressed,
    # regardless of what --compress the spike/membrane recordings themselves used.
...
def main(argument_list=None):
    ...
    # imports `spikecorec` at all -- the binding's read_spire_recording materializes an entire
    # recording in RAM, which is exactly what is impossible at this scale.
    use_streaming = (
