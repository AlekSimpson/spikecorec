...
//
// ── Lateral coupling is OFF by default (and why that is no longer a `--weight 0` placeholder) ─────
// Ticket #131 made AssembledModel dispatch a projection's real synapse ComponentType automatically
// whenever the model declares one — there is no longer a constant-weight dial that can zero out
// propagation while still building the torus's `<projection>` element (dispatch is unconditional
// once a real projection exists). So the torus generated here declares NO projection at all by
// default (`include_lateral_connections=false`, glif_torus_network.h) — the network is a population
// of otherwise-identical, entirely UNCONNECTED neurons, and each driven neuron's output depends only
// on its own bit array, with an exact, unambiguous one-bit-one-spike correspondence. Pass `--couple`
// to rebuild the SAME torus WITH its real 4-neighbor `expOneSynapse` wiring instead (the same
// network the GLIF3/GLIF5 torus examples use) and watch activity spread from both input sites.
//
// Run:  ./build/examples/discrete_spike_input_example [--pattern 0001001010] [--bit-ticks 100]
//                                                     [--amplitude 6e-8] [--couple] [--gbase 10nS]
...
int main(int argument_count, char **argument_values) {
    // Reuses the shared torus flags (ticks/dt/side/gbase/print-ir/verbose).
    TorusExampleOptions options = parse_torus_example_options(
        argument_count, argument_values, TorusExampleOptions{{4000, 1e-4f, false, false}, 8, "10nS", "", false});
    ...
    bool lateral_coupling_enabled = false;
    for (int argument_index = 1; argument_index < argument_count; ++argument_index) {
        String argument = argument_values[argument_index];
        if (argument == "--couple") { lateral_coupling_enabled = true; continue; }
    ...
    // `include_lateral_connections` follows `--couple` (see this file's own header comment for why
    // this replaced the old `--weight 0` placeholder toggle).
    ...
    network_options.include_lateral_connections = lateral_coupling_enabled;
    ...
    std::cout << "]  (" << pattern_bits.size() << " elements, tiled)\n"
              ...
              << "  lateral coupling: " << (lateral_coupling_enabled
                                                ? "ON (--couple) — driven neurons also fire from their neighbours"
                                                : "off (pass --couple) — each neuron reflects only its own bits")
              << "\n\n"
    ...
    if (!lateral_coupling_enabled) {
        std::cout << "\n  Only the two driven neurons fired: lateral coupling is off by default here so\n"
                  << "  the bit-to-spike mapping above is unambiguous. Re-run with --couple to wire the\n"
                  << "  real torus/expOneSynapse network and watch activity spread from both input sites.\n";
    }
