...
//
// ── The weight caveat, stated up front ──────────────────────────────────────────────────────────
// The propagate stage scatters spikes through the k²-tree/WeightMatrix path; it does not yet invoke
// the projection's synapse ComponentType per-edge dynamics (that needs a spike-scatter batch
// construction subsystem no ticket has built). So the scattered value here is an explicit
// PLACEHOLDER current, set with `--weight`, not something derived from the expOneSynapse parameters
// in the model. Topology, routing, and timing are real; the synaptic magnitude is stipulated.
//
// Run:  ./build/examples/glif3_torus_network_example [--side 8] [--ticks 5000] [--weight 2.5e-8]
...
int main(int argument_count, char **argument_values) {
    TorusExampleOptions options = parse_torus_example_options(
        argument_count, argument_values, TorusExampleOptions{{5000, 1e-4f, false, false}, 8, 2.5e-8f});
    ...
    weights.set_constant_weight(options.scattered_weight); // placeholder — see header comment
    ...
    std::cout << "  scattered per-spike placeholder weight: " << options.scattered_weight * 1e12f << " pA\n";
